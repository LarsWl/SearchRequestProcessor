...
encode(text::AbstractString) = GPT2_TOKENIZER(Sentence(text))

function decode(tokens::Vector{TextEncodeBase.TokenStage})::String
    unmap = CodeUnMap(GPT2_TOKENIZER.tokenization.codemap)
...
function get_document_chunks(
    embeddings_adapter::AbstractEmbeddingsAdapter,
    ...
    for doc in documents
        ...
        doc_chunks = filter(chunk -> !isempty(chunk.text), doc_chunks)
    ...
    for i in 1:EMBEDDINGS_BATCH_SIZE:length(all_chunks)
        ...
        batch_embeddings = calculate_embeddings(embeddings_adapter, batch_texts)
